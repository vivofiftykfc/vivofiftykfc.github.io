(function(){
 'use strict';
 const footer=document.querySelector('footer');if(footer&&!footer.querySelector('[data-analytics-privacy]')){const a=document.createElement('a');a.href='/privacy/';a.textContent=' · 访问统计与隐私';a.dataset.analyticsPrivacy='';footer.append(a);}
 ...
 const optout=()=>{try{return localStorage.getItem('hwnote-analytics-optout')==='1';}catch{return false;}};
 ...
 async function send(payload,keepalive=false){const r=await fetch('/__analytics/event',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'omit',body:JSON.stringify(payload),keepalive});if(!r.ok)throw Error('telemetry unavailable');return r.json();}
 async function start(){
  ...
  for(let attempt=0;attempt<2;attempt++){try{const r=await send(body);if(page===item)item.token=r.token;return;}catch{if(attempt===0)await new Promise(r=>setTimeout(r,1500));}}
 }
 ...
 document.addEventListener('hwnote:owner-state',e=>{if(e.detail.authorized){stopped=true;page=null;}});
